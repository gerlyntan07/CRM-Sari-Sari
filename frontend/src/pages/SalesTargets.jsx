...
function SearchableSelect({
  ...
  getLabel,
  placeholder = "Search...",
  disabled = false, // Step 2a: add a disabled prop
}) {
  ...
  return (
    <div ref={ref} className="flex justify-center w-full relative">
      <input
        value={selectedLabel} // Step 2b: always show selected label
        placeholder={placeholder}
        readOnly={true} // Step 2c: prevent typing
        className={`w-full max-w-md border rounded-lg p-1 ${
          disabled
            ? "bg-gray-100 cursor-not-allowed text-gray-500" // gray + not clickable
            : "focus:outline-none focus:ring-1 focus:ring-blue-400 focus:border-blue-400"
        }`}
        onClick={() => {
          if (!disabled) setOpen(!open); // Step 2d: prevent opening dropdown
        }}
      />
...
export default function SalesTargets() {
  ...
  useEffect(() => {
    if (currentUser?.role?.toLowerCase() === "sales" && !formData.user_id) {
  ...
  const handleOpenAddModal = () => {
    // Default form data
    let initialData = { ...INITIAL_FORM_STATE };

    // If the current user is sales, auto-assign user_id
    if (currentUser?.role?.toLowerCase() === "sales") {
      initialData.user_id = String(currentUser.id);
    }
        ...
        <form onSubmit={onSubmit} className="grid gap-5 text-base">
          <div className="flex flex-col gap-1">
            ...
            <SearchableSelect
              ...
              items={users.filter(
                (u) =>
                  u.id === currentUser.id || u.role?.toLowerCase() === "sales"
              )}
              value={formData.user_id}
              onChange={onChange}
              getLabel={(u) => `${u.first_name} ${u.last_name}`}
              placeholder="Search user..."
              disabled={currentUser?.role?.toLowerCase() === "sales"} // <-- disable if sales
            />
          </div>
