          ...
          {actionIcons.map((item, index) => (
            <div key={index} className="relative flex-shrink-0 group">
              <button
                ...
                onClick={() => {
                  const routes = {
                    "Account": "/group-manager/accounts",
                    "Contact": "/group-manager/contacts",
                    "Leads": "/group-manager/leads",
                    "Deals": "/group-manager/deals",
                    "Quotes": "/group-manager/quotes",
                    "Target": "/group-manager/targets",
                    "Task": "/group-manager/tasks",
                    "Meeting": "/group-manager/meetings",
                    "Call": "/group-manager/calls",
                    "Audit": "/group-manager/audit",
                    "Territory": "/group-manager/territory",
                    "SOA": "/group-manager/soas",
                  };
...
const AuditLogItem = ({ action, description, entity_type, entity_name, timestamp }) => {
  ...
  return (
    <li
      ...
      onClick={() => navigate('/group-manager/audit')}
    >
...
const ListCard = ({ title, items, children, onSeeAll }) => {
  ...
  const handleSeeAll = () => {
    if (onSeeAll) {
      ...
      const routeMap = {
        'Latest Leads': '/group-manager/leads',
        'Latest Deals': '/group-manager/deals',
        'Tasks': '/group-manager/tasks'
      };
      if (routeMap[title]) {
...
const LeadItem = ({ first_name, last_name, company_name, source, created_at, id }) => {
  ...
  return (
    <div
      ...
      onClick={() => id && navigate(`/group-manager/leads/${id}`)}
    >
...
const DealItem = ({ name, amount, account, stage, created_at, stage_updated_at, id, previousAmount, currencySymbol }) => {
  ...
  return (
    <div
      className="py-3 border-b border-gray-100 last:border-b-0 cursor-pointer hover:bg-gray-50 transition duration-150 px-2 -mx-2 rounded-md"
      onClick={() => id && navigate(`/group-manager/deals/info?id=${id}`)}
    >
      <div className="flex justify-between items-center">
...
// --- Main TManagerDashboard Component ---

const TManagerDashboard = () => {
  const navigate = useNavigate();
        ...
        allLeads.forEach(lead => {
          ...
          if (name.includes(query) || company.includes(query) || email.includes(query) || phone.includes(query)) {
            results.push({
              ...
              route: `/group-manager/leads/${lead.id}`
            });
          }
        });
      }
        ...
        allDeals.forEach(deal => {
          ...
          if (name.includes(query) || accountName.includes(query) || description.includes(query)) {
            results.push({
              ...
              route: `/group-manager/deals/info?id=${deal.id}`
            });
          }
        });
      }
        ...
        allAccounts.forEach(account => {
          ...
          if (name.includes(query) || email.includes(query) || phone.includes(query) || website.includes(query)) {
            results.push({
              ...
              route: `/group-manager/accounts?id=${account.id}`
            });
          }
        });
      }
  ...
  const metricsConfig = [
    ...
    },
    {
      ...
      color: "text-red-600",
      bgColor: "bg-red-50",
      onClick: () => navigate('/group-manager/tasks')
    },
  ];
